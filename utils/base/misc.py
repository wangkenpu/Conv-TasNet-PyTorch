...
def check(mix_scp, s1_scp, s2_scp):
    mix_key = read_key(mix_scp)
    s1_key = read_key(s1_scp)
    s2_key = read_key(s2_scp)
    check_consistency(mix_key, s1_key)
    check_consistency(mix_key, s2_key)
